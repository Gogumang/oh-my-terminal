...
- **로고가 단색 실루엣으로 보이면 iTerm2의 GPU(Metal) 렌더링을 꺼보세요.** iTerm2 3.6.5
  기본 설정에서는 컬러로 나오는 것을 육안 확인했습니다. 다만 소스상 컬러 폰트 판별이
  `Apple Color Emoji` 폰트 이름 비교로 되어 있어(`iTermCharacterSource.m`), 렌더러 경로에
  따라 알파만 남을 수 있다는 지적이 있습니다. 어느 설정에서 그렇게 되는지는 확인하지
  못했으니, 단색으로 보이면 GPU 렌더링을 끄거나 리거처를 켜 레거시 경로를 쓰세요.
