...
pub fn write_font(base: &Path, brands: &[Brand], output_root: &Path) -> Result<Option<FontArtifact>> {
    ...
    // 한글 코딩 폰트(D2Coding 등)는 .ttc 컬렉션으로 배포된다. 사용자의 주 폰트를
    // 그대로 패치해야 회사 폴더에서 한글이 시스템 폴백으로 떨어지지 않는다.
    let font = match CollectionRef::new(&data) {
        Ok(collection) => collection.get(0).map_err(|error| anyhow!(
            "폰트 컬렉션의 첫 폰트를 읽지 못했다: {error}"))?,
        Err(_) => FontRef::new(&data)?,
    };
    ...
    builder.add_table(&build_name_table())?;
...
/// name 테이블을 표준 인코딩(Windows/Unicode BMP)으로 새로 만든다.
///
/// 원본 레코드를 옮겨 담으면 폰트에 따라 write-fonts가 모르는 인코딩이 섞여 패닉한다
/// (D2Coding이 그랬다). 폰트가 요구하는 최소 이름만 새로 쓰는 쪽이 안전하다.
fn build_name_table() -> write_fonts::tables::name::Name {
